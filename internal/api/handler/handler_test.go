...
func TestProductMetadataHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedStatus:   fiber.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"Either Platform (p) or Package Manager (pm) params must be provided", "status_text":"Bad Request"}`,
			metadata:         models.MetaData{},
			err:              nil,
			version:          "latest",
			version_err:      nil,
		...
		{
			name:             "chef-ice explicit mismatched package manager fails validation",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-ice/metadata?p=ubuntu&m=amd64&pm=rpm&eol=false&v=latest",
			expectedStatus:   fiber.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"Package manager 'rpm' is not valid for platform 'ubuntu'. Expected 'deb', 'tar', or 'zip'", "status_text":"Bad Request"}`,
			metadata:         models.MetaData{},
			err:              nil,
			version:          "latest",
			version_err:      nil,
		...
		},
		{
			...
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-ice/download?eol=false&m=amd64&p=ubuntu&pm=tar&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
				Platform:       "ubuntu",
				PackageManager: "tar",

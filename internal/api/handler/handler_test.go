...
func TestProductsHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			eolParam:         "false",
			expectedStatus:   fiber.StatusOK,
			expectedContains: []string{"chef-360", "chef-ice", "migrate-ice", "inspec-enterprise", "chef-workstation-enterprise"},
		},
		{
		...
		},
		{
			...
			eolParam:         "true",
			expectedStatus:   fiber.StatusOK,
			expectedContains: []string{"chef-360", "chef-ice", "migrate-ice", "inspec-enterprise", "chef-workstation-enterprise", "automate-1"},
		},
		{
			...
			eolParam:         "false",
			expectedStatus:   fiber.StatusOK,
			expectedContains: []string{"automate:Chef Automate", "chef:Chef Infra Client (Legacy)", "chef-server:Chef Infra Server", "chef-workstation:Chef Workstation", "habitat:Chef Habitat", "inspec:InSpec (Legacy)", "chef-ice:Chef Infra Client Enterprise", "migrate-ice:Chef Infra Client Legacy Migration", "inspec-enterprise:InSpec Enterprise", "chef-workstation-enterprise:Chef Workstation Enterprise"},
		},
		{
			...
			eolParam:         "false",
			expectedStatus:   fiber.StatusOK,
			expectedContains: []string{"automate", "chef", "chef-backend", "chef-server", "chef-workstation", "habitat", "inspec", "manage", "supermarket", "chef-360", "chef-ice", "migrate-ice", "inspec-enterprise", "chef-workstation-enterprise"},
		},
	}
...
func TestProductMetadataHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedStatus:   fiber.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"Platform (p) params cannot be empty", "status_text":"Bad Request"}`,
			metadata:         models.MetaData{},
			err:              nil,
			version:          "latest",
			version_err:      nil,
		...
		{
			name:             "chef-ice auto-derives package manager",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-ice/metadata?p=linux&m=amd64&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-ice/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
				Platform:       "linux",
				PackageManager: "rpm",
		...
		{
			name:             "chef-ice missing package manager and platform returns combined error",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-ice/metadata?m=amd64&eol=false&v=latest",
			expectedStatus:   fiber.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"Platform (p) params cannot be empty", "status_text":"Bad Request"}`,
			metadata:         models.MetaData{},
			err:              nil,
		...
		},
		{
			...
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-ice/metadata?p=linux&m=amd64&pm=rpm&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-ice/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
		...
		{
			name:             "chef-ice explicit pm honored regardless of platform mismatch",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-ice/metadata?p=ubuntu&m=amd64&pm=rpm&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-ice/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
		...
		{
			name:             "chef-ice explicit tar package manager is allowed",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-ice/metadata?p=ubuntu&m=amd64&pm=tar&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-ice/download?eol=false&m=amd64&p=linux&pm=tar&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
				Platform:       "linux",
				PackageManager: "tar",
		...
		{
			name:             "migrate-ice auto-derives package manager",
			serverMode:       constants.Trial,
			requestPath:      "/stable/migrate-ice/metadata?p=linux&m=amd64&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/migrate-ice/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
				Platform:       "linux",
				PackageManager: "rpm",
		...
		},
		{
			...
			serverMode:       constants.Trial,
			requestPath:      "/stable/migrate-ice/metadata?p=linux&m=amd64&pm=rpm&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/migrate-ice/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
		...
		{
			name:             "chef-workstation-enterprise auto-derives package manager",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-workstation-enterprise/metadata?p=linux&m=amd64&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-workstation-enterprise/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
				Platform:       "linux",
				PackageManager: "rpm",
		...
		{
			name:             "chef-workstation-enterprise success",
			serverMode:       constants.Trial,
			requestPath:      "/stable/chef-workstation-enterprise/metadata?p=linux&m=amd64&pm=rpm&eol=false&v=latest",
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `{"sha1":"", "sha256":"abcd", "url":"http://example.com/stable/chef-workstation-enterprise/download?eol=false&m=amd64&p=linux&pm=rpm&v=latest", "version":"latest"}`,
			metadata: models.MetaData{
				Architecture:   "amd64",
		...
		{
			name:             "chef-workstation-enterprise failure for opensource server",
			serverMode:       constants.Opensource,
			requestPath:      "/stable/chef-workstation-enterprise/metadata?p=linux&m=amd64&pm=rpm&eol=false&v=latest",
			expectedStatus:   fiber.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"No versions found for this product/mode", "status_text":"Bad Request"}`,
...
func TestProductPackagesHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			serverMode:       constants.Trial,
			requestPath:      "/stable/habitat/packages?eol=false&v=1.6.826",
			expectedStatus:   fiber.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"Product information not found. Please check the input parameters.", "status_text":"Bad Request"}`,
			details: &models.ProductDetails{
				Product:  "habitat",
...
func TestFileNameHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			serverMode:       constants.Commercial,
			expectedStatus:   http.StatusInternalServerError,
			expectedResponse: `{"code":500, "message":"Product information not found. Please check the input parameters.", "status_text":"Internal Server Error"}`,
			metadata:         models.MetaData{},
			version_err:      nil,
			versions:         []string{"19.0.1"},
		},
		{
			...
			serverMode:       constants.Commercial,
			requestPath:      "/current/chef-ice/fileName?p=linux&pv=20.04&m=x86_64&pm=rpm&v=latest",
			expectedStatus:   http.StatusOK,
		...
		},
		{
			...
			serverMode:       constants.Commercial,
			requestPath:      "/current/migrate-ice/fileName?p=linux&pv=20.04&m=x86_64&pm=rpm&v=latest",
			expectedStatus:   http.StatusOK,
		...
		},
		{
			name:             "chef-workstation-enterprise SUCCESS",
			serverMode:       constants.Commercial,
			requestPath:      "/current/chef-workstation-enterprise/fileName?p=windows&pv=pv&m=x86_64&pm=msi&v=latest&license_id=viv2c0a2-111f-2caf-1fa2-1211fe1212d1",
			expectedStatus:   http.StatusOK,
			expectedResponse: `{"fileName":"chef-workstation-enterprise_1.0.0-windows-x86_64.msi"}`,
			metadata:         models.MetaData{FileName: "chef-workstation-enterprise_1.0.0-windows-x86_64.msi"},
			metadata_err:     nil,
			version_err:      nil,
			version:          "",
		...
		{
			name:             "chef-workstation-enterprise not supported for opensource server",
			serverMode:       constants.Opensource,
			requestPath:      "/current/chef-workstation-enterprise/fileName?p=windows&pv=pv&m=x86_64&pm=msi&v=latest",
			expectedStatus:   http.StatusBadRequest,
			expectedResponse: `{"code":400, "message":"No versions found for this product/mode", "status_text":"Bad Request"}`,
			metadata:         models.MetaData{},
			metadata_err:     nil,
		...
		{
			name:             "chef-workstation-enterprise fileName success",
			serverMode:       constants.Commercial,
			requestPath:      "/current/chef-workstation-enterprise/fileName?p=linux&pv=20.04&m=x86_64&pm=rpm&v=latest",
			expectedStatus:   http.StatusOK,
			expectedResponse: `{"fileName":"chef-workstation-enterprise_1.0.0_amd64.deb"}`,
			metadata:         models.MetaData{FileName: "chef-workstation-enterprise_1.0.0_amd64.deb"},
			metadata_err:     nil,
			version:          "latest",
			version_err:      nil,
			versions:         []string{"latest"},

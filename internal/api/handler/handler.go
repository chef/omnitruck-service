...
func (h *DownloadsHandler) ProductsHandler(c *fiber.Ctx) error {
	...
	if !ok {
		return h.SendErrorResponse(c, http.StatusInternalServerError, "Failed to retrieve request injector")
	}
...
func (h *DownloadsHandler) LatestVersionHandler(c *fiber.Ctx) error {
	...
	reqInjector, ok := reqInjectorI.(*do.Injector)
	if !ok {
		return h.SendErrorResponse(c, fiber.StatusInternalServerError, "Failed to retrieve request injector")
	}
	locals := setLocals(c)
	params := helpers.GetRequestParams(c)

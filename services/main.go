...
func (server *ApiService) fetchLatestVersion(params *omnitruck.RequestParams, c *fiber.Ctx) (omnitruck.ProductVersion, *clients.Request) {
	...
	if params.Product == constants.AUTOMATE_PRODUCT || params.Product == constants.HABITAT_PRODUCT {
		...
		if err != nil {
			...
			server.logCtx(c).Error("Error while fetching the latest version for the " + params.Product + " " + err.Error())
...
func (server *ApiService) fetchLatestOSVersion(params *omnitruck.RequestParams, c *fiber.Ctx) (omnitruck.ProductVersion, *clients.Request) {
	...
	if params.Product == constants.AUTOMATE_PRODUCT || params.Product == constants.HABITAT_PRODUCT {
		...
		if err != nil {
			code, msg := getErrorCodeAndMsg(err)
			server.logCtx(c).Error("Error while fetching the latest opensource version for the " + params.Product + " " + err.Error())
			request.Failure(code, msg)
...
func (server *ApiService) fileNameHandler(c *fiber.Ctx) error {
	...
	err, ok := server.ValidateRequest(params, c)
	if !ok {
		server.logCtx(c).Error("Validation of file name API for " + params.Product + " failed" + " " + err.Error())
		return err
	}
...
func (server *ApiService) versionCheckForTrailAndOsServer(params *omnitruck.RequestParams, c *fiber.Ctx) error {
	if server.Mode == Trial {
		...
		if isLatest(params.Version) {
			...
			if !err.Ok {
				server.logCtx(c).Error("Error while fetching latest opensource version for the product " + params.Product + " " + errors.New(err.Message).Error())
				return fiber.NewError(err.Code, err.Message)
			}

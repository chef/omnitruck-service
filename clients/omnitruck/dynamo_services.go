...
func (svc *DynamoServices) Products(products []string, eol string) []string {
	products = append(products, constants.HABITAT_PRODUCT, constants.CHEF_INFRA_CLIENT_ENTERPRISE_PRODUCT, constants.MIGRATE_ICE, constants.CHEF_INSPEC_ENTERPRISE_PRODUCT)
	if eol == "true" {
